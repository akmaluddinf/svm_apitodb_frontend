...
function App() {
  ...
  const handleFlagChange = (id, currentFlag) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        axios.patch(`http://localhost:3002/api/endpoints/${id}`, { endpoints_read: newFlag })
          .then(response => {
  ...
  const handleUpdateAllFlags = (newFlag) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        axios.patch('http://localhost:3002/api/endpoints/updateAllFlag', { endpoints_read: newFlag })
          .then(response => {
            console.log(response.data.message);
            fetchData();
  ...
  const handleFlagChangeDataPeriode = (id, currentFlag) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        axios.patch(`http://localhost:3002/api/dataPeriode/${id}`, { flag: newFlag })
          .then(response => {
            console.log(response.data.message);
            fetchDataPeriode();
  ...
  const handleUpdateAllFlagsDataPeriode = (newFlag) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        axios.patch('http://localhost:3002/api/dataPeriode/updateAllFlag', { flag: newFlag })
          .then(response => {
            console.log(response.data.message);
            fetchDataPeriode();
                    ...
                    <table className="table table-bordered table-hover text-nowrap table-sm">
                      ...
                      <tbody>
                        {currentItemEndpoint.map((endpoint, index) => (
                          <tr
                            key={endpoint.endpoints_id}
                            style={{
                              background: endpoint.endpoints_read === 1 ? '#26ff000f' : '#f3444414',
                              color: 'black',
                            }}
                          >
                            ...
                            <td>
                              {getFlagStatus(endpoint.endpoints_read)}
                            </td>
                            <td style={{ textAlign: "center" }}>
                              {endpoint.endpoints_read === 1 ? (
                                <FontAwesomeIcon icon={faHippo} onClick={() => handleFlagChange(endpoint.endpoints_id, endpoint.endpoints_read)} style={{ cursor: "pointer", color: "red", height: "26px" }} data-toggle="tooltip" title="Set Status Read Non Aktif" data-placement="top" />
                              ) : (
                                <FontAwesomeIcon icon={faCat} onClick={() => handleFlagChange(endpoint.endpoints_id, endpoint.endpoints_read)} style={{ cursor: "pointer", color: "00a65a", height: "26px" }} data-toggle="tooltip" title="Set Status Read Aktif" data-placement="top" />
                              )}
                            </td>
                          </tr>
                        ))}
                      ...
                      <tbody>
                        {currentItemDataPeriode.map((item, index) => (
                          ...
                          >
                            ...
                            <td>
                              {getFlagStatusDataPeriode(item.flag)}
                            </td>
                            <td style={{ textAlign: "center" }}>
                              {item.flag === 1 ? (
                                <FontAwesomeIcon icon={faHippo} onClick={() => handleFlagChangeDataPeriode(item.fid, item.flag)} style={{ cursor: "pointer", color: "red", height: "26px" }} data-toggle="tooltip" title="Set Status Read Non Aktif" data-placement="top" />
                              ) : (
                                <FontAwesomeIcon icon={faCat} onClick={() => handleFlagChangeDataPeriode(item.fid, item.flag)} style={{ cursor: "pointer", color: "00a65a", height: "26px" }} data-toggle="tooltip" title="Set Status Read Aktif" data-placement="top" />
                              )}
                            </td>
                          </tr>
                        ))}
